...
class AddRoleHandler @Inject constructor() : Handler() {
    companion object {
    ...
    override fun subscribe(): Disposable {
        return manager.on<MessageReceivedEvent>()
            .map { it.message }
            .filter { it.channel.isCommandChannel && it.contentRaw.startsWith("!addrole") }
            .flatMap {
                ...
                val pAddRole = it.mentionedMembers.toFlux().flatMap { member ->
                    allowedRoles.toFlux().flatMap(member::addRole)
                }
                ...
                Flux.concat(pAddRole, pAnswer, pLogStaff)
            }
            .subscribeDefault()
    }
}
